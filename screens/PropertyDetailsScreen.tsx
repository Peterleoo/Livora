...
export const PropertyDetailsScreen: React.FC<PropertyDetailsScreenProps> = ({
      ...
      >
         <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm">
            <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2">
               <Icon name="psychology" className="text-indigo-600" size={20} />
               AI 定制分析报告
            </h3>
   ...
   const LocationDetails = () => (
      ...
      >
         <div className="bg-slate-200 h-48 rounded-3xl overflow-hidden relative">
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-2xl shadow-lg border border-white">
   ...
   return (
      <div className="flex flex-col h-[100dvh] bg-[#f8fafc] relative overflow-hidden">
         {/* Top Icons Container */}
         <div className="absolute top-0 left-0 right-0 p-6 z-30 flex items-center justify-between pointer-events-none">
            <motion.button
               whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
               onClick={onBack}
               className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-xl flex items-center justify-center text-white border border-white/20 pointer-events-auto mt-safe-top"
            >
               <Icon name="arrow_back" />
            </motion.button>

            <div className="flex gap-4 pointer-events-auto mt-safe-top">
               <motion.button
                  whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
         ...
         <div className="relative h-[45vh] w-full shrink-0">
            ...
            <div className="absolute bottom-10 left-6 right-6">
               ...
               <motion.h1
                  ...
                  className="text-3xl font-bold text-slate-900 drop-shadow-sm"
               >
                  {property.title}
               </motion.h1>
            </div>
         ...
         <div className="flex-1 overflow-y-auto px-6 -mt-6 relative z-10 scroll-smooth pb-44">
            {/* AI Insight Highlight Row */}
            <motion.div
               initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
               className="bg-white/80 backdrop-blur-2xl border border-white rounded-3xl p-5 shadow-xl shadow-slate-200/50 mb-6 flex items-center gap-6"
            >
            ...
            {/* Tabs */}
            <div className="flex bg-slate-100/80 p-1 rounded-2xl mb-8 sticky top-0 z-20 backdrop-blur-md">
               {(['INFO', 'AI', 'LOCATION'] as const).map(tab => (
                  <button
                     key={tab}
                     onClick={() => setActiveTab(tab)}
                     className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === tab ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
                  >
               ...
               {activeTab === 'INFO' && (
                  ...
                  >
                     ...
                     <div className="bg-white p-5 rounded-3xl border border-slate-50 shadow-sm">
                        <h3 className="text-sm font-bold text-slate-900 mb-3">房源简介</h3>
         ...
         {/* 3. Sticky Action Bar */}
         <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/95 to-transparent z-40 border-t border-slate-50 backdrop-blur-xl">
            <div className="flex items-center gap-4 pb-safe-bottom">
               <div className="flex-1">
                  ...
                  <div className="text-2xl font-black text-slate-900">¥{property.price.toLocaleString()}<span className="text-sm font-medium text-slate-400">/月</span></div>
               </div>
         ...
         {/* 4. VR Preview Entry Trigger */}
         <motion.button
            initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.8 }}
            onClick={() => onEnterVR()}
            className="absolute right-6 bottom-32 w-16 h-16 rounded-full bg-slate-900 shadow-2xl flex items-center justify-center text-white border-4 border-white z-50 overflow-hidden"
         >
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 10, repeat: Infinity, ease: 'linear' }} className="absolute inset-0 opacity-20 bg-gradient-to-tr from-indigo-500 to-transparent" />
            <Icon name="360" size={28} />
         </motion.button>
      </div>
   );
};
