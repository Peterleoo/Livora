...
export const ContractDetailsScreen: React.FC<ContractDetailsScreenProps> = ({ contract, onBack }) => {
    ...
    const DepositModal = ({ onClose }: { onClose: () => void }) => {
        ...
        return (
            ...
            >
                <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl pt-safe-top p-4 shadow-sm relative border-b border-slate-100 dark:border-slate-800">
                    <button onClick={onClose} className="absolute left-4 bottom-4 p-2 -ml-2"><Icon name="close" /></button>
                    <h2 className="text-center font-bold text-lg mt-1">押金管理</h2>
                </div>
                ...
                <div className="flex-1 p-5 overflow-y-auto">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm text-center mb-6 relative overflow-hidden">
                        ...
                        <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold mt-4 ${contract.depositStatus === 'HOSTED' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'
                            }`}>
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-[#101010]">
            {/* Header */}
            <div className="px-4 py-3 pt-safe-top flex items-center gap-2 bg-white/70 dark:bg-[#1a1a1a]/70 backdrop-blur-xl sticky top-0 z-10 border-b border-slate-100 dark:border-slate-800">
                <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-slate-50/50 dark:hover:bg-[#202020]/50">
